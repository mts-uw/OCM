...
elements = pd.DataFrame(columns = desc.index)
...
ax1.hist(data.loc[:,'Temperature, K'],bins = edge, ec = 'black', color = 'orange', label = '2010-2019')
...
ax2.hist(data.loc[:,'p(CH4)/p(O2)'],bins = edge, ec = 'black', color = 'orange', label = '2010-2019')
...
ax3.hist(np.log10(data.loc[:,'Contact time, s']), bins = edge,ec = 'black', color = 'orange', label = '2010-2019')
ax3.hist(np.log10(data_.loc[:,'Contact time, s']),bins = edge, ec = 'black', label = '-2009')
...
bar_ = (data_.loc[:,'Li':'Bi'] > 0).sum()
topk =25
labels = bar.index
indices = np.argsort(bar)[::-1]
topk_idx = indices[:topk]
ax4.bar(range(len(topk_idx)), bar[topk_idx], color = 'orange', align = 'center', ec = 'black', label = '2010-2019')
ax4.bar(range(len(topk_idx)), bar_.loc[labels[topk_idx]], ec = 'black', label = '-2009')
...
bar_ = (data_.loc[:,"Li":"Bi"] > 0).sum(axis =1)
labels = np.arange(1, 9)
count = {}
for i in range(1,9):
    count[i] = (bar == i).sum()
    
ax5.bar(range(1,9), count.values() ,color ='orange', ec = 'black', label = '2010-2019')    
count = {}
for i in range(1,9):
    count[i] = (bar_ == i).sum()
...
plt.savefig("out/Figure_1.png", format = "png", dpi = 1000)
plt.close()
...
topk_idx = indices[:topk]
plt.barh(range(len(topk_idx)), bar[topk_idx], color = 'orange', align = 'center', ec = 'black', label = '2010-2019')
plt.barh(range(len(topk_idx)), bar_.loc[labels[topk_idx]], ec = 'black', label = '-2009')
plt.yticks(range(len(topk_idx)), labels[topk_idx])
plt.legend(loc = 'lower right')
plt.xlabel('Datapoints')
plt.savefig('out/Figure_1_sub.png', dpi = 1200, bbox_inches = 'tight')
plt.close()
...
plt.savefig("out/Figure_S3.png", format = "png",  dpi = 1000)
plt.close()
...
color = "blue"
xx, yy, zz =  data.loc[idx, "X(CH4), %"], data.loc[idx, "S(C2), %"], data.loc[idx, "Y(C2), %"]
...
idx = ((data.loc[:,'W']== 0) & (data.loc[:,'Mn'] == 0)) & ((data.loc[:,'Li'] == 0) & (data.loc[:,'Mg'] == 0)) & ((data.loc[:,'La'] == 0) )
color = "red"
xx, yy, zz =  data.loc[idx, "X(CH4), %"], data.loc[idx, "S(C2), %"], data.loc[idx, "Y(C2), %"]
...
ax_main.legend()
fig.savefig("out/Figure_2_2.png", format = "png", dpi = 1200) 
plt.close()
...
idx = (data.loc[:, "Temperature, K"] >= 700 +273.15) & (data.loc[:, "Temperature, K"] < 800 +273.15)
color = "orange"
xx, yy, zz =  data.loc[idx, "X(CH4), %"], data.loc[idx, "S(C2), %"], data.loc[idx, "Y(C2), %"]
...
idx = (data.loc[:, "Temperature, K"] >= 800 +273.15) & (data.loc[:, "Temperature, K"] < 900 +273.15)
color = "green"
xx, yy, zz =  data.loc[idx, "X(CH4), %"], data.loc[idx, "S(C2), %"], data.loc[idx, "Y(C2), %"]
...
ax_main.legend()
fig.savefig("out/Figure_2_3.png", format = "png", dpi = 1200)
plt.close()
...
ax_main.legend()
fig.savefig("out/Figure_2_4.png", format = "png", dpi = 1200)
plt.close()
